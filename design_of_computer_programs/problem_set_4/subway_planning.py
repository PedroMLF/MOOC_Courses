...
def subway(**lines):
    ...
    def pairs(list_object):
        return [ list_object[ix:ix+2] for ix in range(len(list_object) - 1) ]
...
def ride(here, there, system=boston):
    ...
    def subsuccessors(station):
        return system[station]

    return shortest_path_search(here, subsuccessors, is_goal)

def longest_ride(system):
    """"Return the longest possible 'shortest path' 
    ride between any two stops in the system."""
    ## your code here
    end = [station for station in system if len(system[station])==1]
    return max([ride(a, b, system) for a in end for b in end], key=len)
