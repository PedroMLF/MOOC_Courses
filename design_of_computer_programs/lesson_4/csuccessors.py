...
def csuccessors(state):
    ...
    # Norvig Solution, with corrections for the bug he mentioned
	if C1 > M1 > 0 or C2 > M2 > 0:
		return {}
	items = []
	if B1 > 0:
		items += [(sub(state, delta), a + "->")
					for delta, a in deltas.items() if all(res >= 0 for res in sub(state, delta))]
	if B2 > 0:
		items += [(add(state, delta), "<-" + a)
					for delta, a in deltas.items() if all(res >= 0 for res in add(state, delta))]
	return dict(items)
